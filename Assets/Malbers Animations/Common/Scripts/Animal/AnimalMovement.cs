    ...
    public partial class Animal
    {
        ...
        protected virtual void FixPosition(float time)
        {
            ...
            if (pivot_Hip && pivot_Chest)
            {
                ...
                Quaternion finalRot = Quaternion.FromToRotation(transform_Up, surfaceNormal) * transform.rotation;  //Calculate the orientation to Terrain  
                ...
                if (isInAir || IsJumping() || !backray)                                     // If the character is falling, jumping or swimming smoothly aling with the Up Vector
                {
                    ...
                    {
                        finalRot = Quaternion.FromToRotation(transform_Up, UpVector) * transform.rotation;
