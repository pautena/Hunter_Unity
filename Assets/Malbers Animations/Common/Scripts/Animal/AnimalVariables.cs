...
namespace MalbersAnimations
{
    ...
    public partial class Animal
    {
        ...
        protected Renderer animalMesh;              //Reference for the Mesh Renderer of this animal
        #endregion
        ...
        protected int
            ...
            loops = 1;              //Some Attacks can have multiple loops like the wing attack from the dragon
        #endregion
        ...
        Quaternion DeltaRotation;                             //Store the last Rotation before making all the rotations
       //float UnderWaterShift = 1;                           //Shift  
        ...
        {
            get { return CurrentAnimState.tagHash == Hash.Fall; }
        }
        ...
        {
            get { return CurrentAnimState.tagHash == Hash.Tag_Recover; }
        }
        ...
        public bool Fly
        {
            ...
            set
            {
                ...
                //if (stand) { return; }                          //Do nothing if canfly is false
            ...
            set
            {
                ...
                if (!isAttacking)
                {
                    if (value)                                                              //If Attack was set to true
                    {
                        ...
                        StartCoroutine(C_Attacking(attackDelay));
                        OnAttack.Invoke();
                    }
                }

                if (attackDelay <= 0)                               //In case the attacks are set to not be interrupted
                {
                    if (!RealAnimatorState(Hash.Tag_Attack))
                    {
                        isAttacking = false;
                    }
                }
        ...
        public bool Action
        {
            ...
            set
            {
                if (RealAnimatorState(Hash.Action)) return;         //Don't Make an New action if you already are making an action
                //if (RealAnimatorState(Hash.Tag_Jump)) return;       //Do not play any action if you are jumping.
